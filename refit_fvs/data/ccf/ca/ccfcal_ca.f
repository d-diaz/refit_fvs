      SUBROUTINE CCFCAL(ISPC,D,H,JCR,P,LTHIN,CCFT,CRWDTH,MODE,IFOR)
      IMPLICIT NONE
C----------
C CA $Id$
C----------
...
C    IFOR = 5 IF IN USFS REGION 5
C           6 IF IN USFS REGION 6
C----------
COMMONS
C
C
...
Cf2py integer,intent(in) :: IFOR
Cf2py real,intent(in) :: D
Cf2py real,intent(in) :: H
...
C----------
      CALL R5CRWD (VARACD,ISPC,D,H,CRWD5)
C----------
...
C----------
  100 CONTINUE
      CALL R6CRWD (VARACD,ISPC,D,H,CRWDTH)
  200 CONTINUE
C----------
